...
export class VkProvider extends SocialAbstract implements SocialProvider {
  ...
  async pages(accessToken: string) {
    const user = await (
      await this.fetch(
        `https://api.vk.com/method/users.get?v=5.251&access_token=${accessToken}&fields=photo_200,screen_name`
      )
    ).json();

    const groups = await (
      await this.fetch(
        `https://api.vk.com/method/groups.get?v=5.251&access_token=${accessToken}&extended=1&filter=admin,editor,moderator&fields=photo_200,screen_name`
      )
    ).json();

    const userPage = {
      id: String(user.response[0].id),
      name: `${user.response[0].first_name} ${user.response[0].last_name}`,
      picture: user.response[0].photo_200,
      username: user.response[0].screen_name || String(user.response[0].id),
    };

    const groupPages = (groups.response?.items || []).map((group: any) => ({
      id: String(-group.id),
      name: group.name,
    ...
    return [userPage, ...groupPages];
  }

  async fetchPageInformation(accessToken: string, data: any) {
    return {
      id: data.id,
      name: data.name,
  ...
  private async uploadMedia(
    ...
    return await Promise.all(
      (post?.media || []).map(async (media) => {
        ...
        const isGroup = userId.startsWith('-');

        const all = await (
          await this.fetch(
            isVideo
              ? `https://api.vk.com/method/video.save?access_token=${accessToken}&v=5.251${isGroup ? `&group_id=${Math.abs(Number(userId))}` : ''
              }`
        ...
        const slash = media.path.split('/').slice(-1)[0];
